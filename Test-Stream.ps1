<#
    .DESCRIPTION
    WinForms IPTV stream launcher that parses M3U playlist files and launches streams via
    mpv.exe. Requires PowerShell 5.1. Reads All_Stations.m3u from the script's directory
    by default (resolved via $PSScriptRoot or $PSCommandPath fallbacks). The Parse-M3U
    function extracts channel names from #EXTINF entries and their stream URLs. Provides
    a searchable channel list in a ListBox; double-clicking or pressing Enter launches the
    selected stream by passing the URL to mpv.exe. Favorites are persisted as JSON at
    %APPDATA%\M3UStreamLauncher\favorites.json (created automatically). The UI includes
    a search textbox, a favorites toggle button, and a "Load M3U" button to open a
    different playlist file. Intended to run on a workstation with mpv.exe on PATH or
    with $mpvPath set to the full exe path.

    Example:
        # Requires mpv.exe installed and All_Stations.m3u in the same folder
        .\Test-Stream.ps1
        # Opens the IPTV launcher; search channels, double-click to play in mpv
#>
...
# ---------------- CONFIG ----------------
$mpvPath = "mpv.exe"   # set full path if needed
